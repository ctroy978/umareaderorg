...
from agents.graphs.comprehension_coach.prompts.planner import PLANNER_SYSTEM, PLANNER_USER

_VALID_ACTIONS = {"prompt_generator", "response_evaluator", "end"}
...
def planner_node(state: ComprehensionCoachState) -> dict:
    llm = get_planner_llm()

    user_msg = PLANNER_USER.format(
        student_response_status=_status(state.get("student_response")),
        generated_prompt_status=_status(state.get("generated_prompt")),
        response_assessment_status=_status(state.get("judge_feedback", {}).get("response_assessment")),
